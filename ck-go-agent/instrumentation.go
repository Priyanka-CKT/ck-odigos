...
const (
	...
	envCkClientPathOverrideKey = "CK_CLIENT_PATH_OVERRIDE"
	// envCkExporterKey is the key for the environment variable value
	// containing what OpenTelemetry trace exporter to use.
	envCkExporterKey    = "CK_NEXUS_ENDPOINT"
	envCkPushGatewayKey = "CK_PG_ENDPOINT"
	envCkServiceNameKey = "APP_NAME"
	envCkClusterNameKey = "CK_CLUSTER_NAME"
	envCkNamespaceKey   = "CK_NAMESPACE"
	envCkPodNameKey     = "CK_POD_NAME"
	// defaultEndpointFormat is the default endpoint format value for the trace exporter.
	defaultCkExporterKey    = "http://ckn-nexus-app:8081"
	defaultCkPushGatewayKey = "http://cktest-pushgateway:9091"
)
...
func NewInstrumentation(ctx context.Context, opts ...InstrumentationOption) (*Instrumentation, error) {
	...
	ckendpoint := defaultCkExporterKey
	if e, ok := lookupEnv(envCkExporterKey); ok {
		ckendpoint = e
	}
	//ckendpoint = fmt.Sprintf("%s/api/graph-paths/%s", ckendpoint, c.serviceName)

	pushGateway := defaultCkPushGatewayKey
	if e, ok := lookupEnv(envCkPushGatewayKey); ok {
		pushGateway = e
	}

	clusterName := ""
	if e, ok := lookupEnv(envCkClusterNameKey); ok {
		clusterName = e
		fmt.Printf("clusterName in env: %s\n", clusterName)
	}

	namespace := ""
	podName := ""

	// First try to get values from additionalResAttrs
	for _, attr := range c.additionalResAttrs {
		switch attr.Key {
	...
	clientOverridePaths := ""
	if e, ok := lookupEnv(envCkClientPathOverrideKey); ok {
		clientOverridePaths = e
		fmt.Printf("clientOverridePaths in env: %s\n", clientOverridePaths)
	}

	// Create factory once
	exporterConfigs := []telemetry.ExporterConfig{
		{
	...
	factory, err := telemetry.NewFactory(c.logger, exporterConfigs)
	if err != nil {
		return nil, err
	}

	cp := convertConfigProvider(c.cp)
	mngr, err := instrumentation.NewManager(c.logger, ctrl, c.globalImpl, cp, Version(), factory, c.serviceName, clientOverridePaths)
	if err != nil {
		return nil, err
	}
...
func lookupResourceData() (string, []attribute.KeyValue, bool) {
	// Prioritize APP_NAME over OTEL_SERVICE_NAME over OTEL_RESOURCE_ATTRIBUTES value.
	svcName := ""
	var source string

	// First check for APP_NAME (highest priority)
	if v, ok := lookupEnv(envCkServiceNameKey); ok {
		svcName = v
		source = "APP_NAME"
	} else if v, ok := lookupEnv(envServiceNameKey); ok {
		// Then check for OTEL_SERVICE_NAME (second priority)
		svcName = v
	...
	if !ok {
		if svcName != "" {
			log.Printf("Using service name '%s' from %s", svcName, source)
		}
	...
	for _, keyval := range strings.Split(strings.TrimSpace(v), ",") {
		...
		if key == string(semconv.ServiceNameKey) {
			// Only use service.name from resource attributes if APP_NAME and OTEL_SERVICE_NAME are not set
			if svcName == "" {
