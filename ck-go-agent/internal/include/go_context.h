...
#define MAX_CONCURRENT_SPANS 10000
...
static __always_inline void *get_parent_go_context(struct go_iface *go_context, void *map) {
    ...
    bpf_debug_printk("get_parent_go_context: go_context: %p, data: %p", go_context, data);
    for (int i = 0; i < MAX_DISTANCE; i++)
    {
        if (data == NULL)
        {
            bpf_debug_printk("get_parent_go_context: data is NULL");
            break;
        }
        ...
        {
            bpf_debug_printk("get_parent_go_context: Parent found in map : %p", data);
            return data;
        }
...
static __always_inline struct span_context *get_parent_span_context(struct go_iface *go_context) {
    bpf_debug_printk("get_parent_span_context: looking for parent in map : go_context_to_sc");
    void *parent_go_ctx = get_parent_go_context(go_context, &go_context_to_sc);
    if (parent_go_ctx == NULL)
    {
        bpf_debug_printk("get_parent_span_context: parent_go_ctx is NULL. go_context_data Key: %p", go_context->data);
        return NULL;
    }
    ...
    if (parent_sc == NULL)
    {
        bpf_debug_printk("get_parent_span_context: parent_sc is NULL. go_context_data Key: %p", go_context->data);
        return NULL;
    }
...
static __always_inline void start_tracking_span(void *contextContext, struct span_context *sc) {
    long err = 0;
    bpf_debug_printk("start_tracking_span: map : go_context_to_sc , key: go_context.data %p, value type: span_context", contextContext );
    err = bpf_map_update_elem(&go_context_to_sc, &contextContext, sc, BPF_ANY);
    if (err != 0)
    {
        bpf_debug_printk("Failed to update tracked_spans map: %ld", err);
        return;
    }

    bpf_debug_printk("start_tracking_span: map : tracked_spans_by_sc , key: span_context %p, value type: context", sc );
    err = bpf_map_update_elem(&tracked_spans_by_sc, sc, &contextContext, BPF_ANY);
    if (err != 0)
    {
        bpf_debug_printk("Failed to update tracked_spans_by_sc map: %ld", err);
        return;
    }
}
...
static __always_inline void stop_tracking_span(struct span_context *sc, struct span_context *psc) {
    if (sc == NULL) {
        bpf_debug_printk("stop_tracking_span: sc is null");
        return;
    }
    ...
    if (parent_ctx == NULL)
    {
        ...
        bpf_debug_printk("stop_tracking_span: delete map : go_context_to_sc , key: go_context.data %p, value type: span_context", ctx );
    ...
    } else 
    {
        ...
        if (ctx_val != parent_ctx_val)
        {
            ...
            bpf_debug_printk("stop_tracking_span: update map : go_context_to_sc , key: go_context.data %p, value: span_context", ctx );
            bpf_map_update_elem(&go_context_to_sc, ctx, psc, BPF_ANY);
        }
    }

    bpf_debug_printk("stop_tracking_span: delete map : tracked_spans_by_sc , key: span_context %p, value type: context", sc );
...
static __always_inline void get_Go_context_from_struct(void *ctx_type_or_struct, const volatile u64 context_offset, struct go_iface *contextContext) {
    void *context_struct_ptr = (void*)(ctx_type_or_struct + context_offset);
    bpf_probe_read(&contextContext->type, sizeof(contextContext->type),context_struct_ptr );
    bpf_probe_read(&contextContext->data, sizeof(contextContext->data), get_go_interface_instance(context_struct_ptr));
}
