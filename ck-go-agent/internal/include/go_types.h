...
static __always_inline struct go_string write_user_go_string(char *str, u32 len)
{
    ...
    if (addr == NULL) {
        bpf_debug_printk("write_user_go_string: failed to copy string to userspace");
        return new_string;
    }
...
static __always_inline void append_item_to_slice(void *new_item, u32 item_size, void *slice_user_ptr)
{
    ...
    if (slice_len < slice_cap && slice.array != NULL)
    {
        ...
        bpf_debug_printk("append_item_to_slice: successfully appended to slice with address: %p", slice.array);
    }
    else
    { 
        ...
        if (!map_buff)
        {
            bpf_debug_printk("append_item_to_slice: failed to get slice_array_buff");
            return;
        }
        ...
        {
            bpf_debug_printk("append_item_to_slice: no room for new item. Alloc size: %d, item size: %d, max slice array size: %d", alloc_size, item_size, MAX_SLICE_ARRAY_SIZE);
            return;
        }
        // Append to buffer
        if (slice.array != NULL) {
            bpf_probe_read_user(new_slice_array, alloc_size, slice.array);
            bpf_debug_printk("append_item_to_slice: copying %d bytes to new array from address 0x%llx", alloc_size, slice.array);
        }
        ...
        bpf_debug_printk("append_item_to_slice: new array size: %d, new array address: 0x%llx", new_array_size, new_array);
    ...
    bpf_debug_printk("append_item_to_slice: successfully updated slice in userspace");
}
...
static __always_inline u32 get_go_string_from_user_ptr_with_len(void *user_str_ptr, char *dst, u64 max_len)
{
    if (user_str_ptr == NULL)
    {
        return 0;
