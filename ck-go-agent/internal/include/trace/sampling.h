...
#define MAX_INS_CONFIG_VALUE_SIZE 32
...
//contains the instrumentation config
struct {
	__uint(type, BPF_MAP_TYPE_HASH);
	__type(key, u8);
    __type(value, u8[MAX_INS_CONFIG_VALUE_SIZE]);
	__uint(max_entries, 2);
    __uint(pinning, LIBBPF_PIN_BY_NAME);
} instrumentation_config_map SEC(".maps");

static const u8 FLAG_SAMPLED = 1;

static __always_inline bool is_ck_instrumentation_enabled()
{
    u8 ins_config_key = 0;
    u8 *ins_value = bpf_map_lookup_elem(&instrumentation_config_map, &ins_config_key);
    if(ins_value == NULL){
        bpf_debug_printk("instrumentation config is not set");
        return true; //default to enabled
    }
    bpf_debug_printk("instrumentation config: %d", *ins_value);
    return *ins_value == 1;
}
