...
func processFnC(e *event, logger *slog.Logger, serviceName string) telemetry.PathKey {
	logger.Info("=== Processing Kafka Consumer event", "event", e)
	...
	pathKey := telemetry.PathKey{
		...
		GraphPathNode: telemetry.GraphPathNode{
			...
			Runtime:      "go",
			GraphPathElement: telemetry.KafkaConsumerElement{
				Type:      "KafkaConsumer",
				GroupId:   unix.ByteSliceToString(e.ConsumerGroup[:]),
				TopicName: unix.ByteSliceToString(e.Topic[:]),
			},
			StartTime: e.StartTime,
			EndTime:   e.EndTime,
			IsError:   false, //There is no concept of error in kafka consumer
			ErrorCode: "",
		},
	}

	logger.Info("=== Created Kafka Consumer PathKey", "pathKey", pathKey)
	return pathKey
}
