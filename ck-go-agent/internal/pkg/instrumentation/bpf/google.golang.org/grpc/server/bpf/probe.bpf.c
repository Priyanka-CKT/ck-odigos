...
#define GRPC_METHOD_MAX_LEN GRPC_METHOD_SIZE
...
struct grpc_request_t
{
    ...
    char method[GRPC_METHOD_MAX_LEN];
    u32 status_code;
    net_addr_t local_addr;
    u8 has_status;
    bool is_error; //This is never set. It is just used so that the UPROBE_RETURN macro can be used.
    u16 error_code; //This is never set. It is just used so that the UPROBE_RETURN macro can be used.
};
...
static __always_inline int handleStream(struct pt_regs *ctx, void *stream_ptr, struct go_iface *go_context) {
    ...
    if (go_context == NULL) {
        bpf_debug_printk("grpc:server:handleStream: NULL go_context");
        return -1;
    }

    if (stream_ptr == NULL) {
        bpf_debug_printk("grpc:server:handleStream: NULL stream_ptr");
        return -1;
    }
    ...
    if (grpcReq_event_ptr != NULL) {
        bpf_debug_printk("grpc:server:handleStream: event already tracked");
        return 0;
    }
    ...
    if (rc != 0) {
        bpf_debug_printk("grpc:server:handleStream: failed to read stream ID");
        return -2;
    }
    ...
    if (grpcReq == NULL) {
        ...
        if (grpcReq == NULL) {
            bpf_debug_printk("grpc:server:handleStream: failed to get grpcReq");
            return 0;
        }
    ...
    if (!parsed_method ) {
        bpf_debug_printk("grpc:server:handleStream: failed to read gRPC method from stream");
    ...
    bytes_to_hex_string(grpcReq->psc.TraceID, MAX_PATH_KEY_BYTE, inc_path_key);

    int len_to_copy = GRPC_METHOD_MAX_LEN;
    char *method_str = grpcReq->method;
    if (grpcReq->method[0] == '/') {
        method_str = grpcReq->method + 1;
        len_to_copy = GRPC_METHOD_MAX_LEN - 1;
    }
    char method_str_for_hash[GRPC_METHOD_MAX_LEN] = {};
    copy_byte_arrays(method_str, method_str_for_hash, len_to_copy);
    

    //This is crazy. If I remove this print or make it into debug print, it will fail
    //saying stack size exceeded. Bloody hell.
    // bpf_printk("grpc server: ipath=%s grpc_method=%s",inc_path_key, grpc_method);


    u32 inc_path_key_hash = fnv1a_hash32(inc_path_key, MAX_PATH_KEY_CHAR);
    u32 grpc_method_hash = fnv1a_hash32(method_str_for_hash, GRPC_METHOD_MAX_LEN);
    u32 service_name_hash = service_name_fnv1a_hash32();
    u32 hashes[3] = {service_name_hash, inc_path_key_hash, grpc_method_hash};
    combine_to_16byte_hash(hashes, 3, path_key);
    copy_byte_arrays(path_key, grpcReq->sc.TraceID, MAX_PATH_KEY_BYTE);
    
    // Print debug info
    bytes_to_hex_string(path_key, MAX_PATH_KEY_BYTE, inc_path_key);
    bpf_debug_printk("grpc server: Generated pathkey : %s", inc_path_key);
    ...
    if (server_addr_supported) {
        ...
        if (http2server != NULL) {
            ...
            bpf_debug_printk("grpc:server:handleStream: failed to get http2server arg");
        }
    }
    ...
    if (rc != 0) {
        bpf_debug_printk("grpc:server:handleStream: failed to update event. rc: %d", rc);
        return -4;
    }
...
static __always_inline int writeStatus(struct pt_regs *ctx, void *status_ptr) {
    if (status_ptr == NULL) {
        bpf_debug_printk("grpc:server:writeStatus: NULL status_ptr");
        return -1;
    }
    ...
    if (req_ptr == NULL) {
        bpf_debug_printk("grpc:server:handleStream: failed to lookup grpc request");
        return -2;
    }
    ...
    if (rc != 0) {
        bpf_debug_printk("grpc:server:handleStream: failed to read Status.s");
        return -3;
    }
    ...
    if (rc != 0) {
        bpf_debug_printk("grpc:server:handleStream: failed to read status code. rc: %d", rc);
        return -4;
    }
...
int uprobe_server_handleStream(struct pt_regs *ctx) {
    bpf_debug_printk("=== uprobe_server_handleStream called with ctx: %p ===", ctx);
    u64 stream_pos = 4;
...
UPROBE_RETURN(server_handleStream, struct grpc_request_t, grpc_events, 0)
...
int uprobe_server_handleStream2(struct pt_regs *ctx) {
    bpf_debug_printk("=== uprobe_server_handleStream2 called with ctx: %p ===", ctx);
    u64 server_stream_pos = 4;
    void *server_stream_ptr = get_argument(ctx, server_stream_pos);
    if (server_stream_ptr == NULL) {
        bpf_debug_printk("grpc:server:uprobe/server_handleStream2: failed to get ServerStream arg");
        return -1;
    }
    ...
    if (rc != 0) {
        bpf_debug_printk("grpc:server:uprobe/server_handleStream2: failed to read stream_ptr");
        return -2;
    }
    ...
    if (rc != 0) {
        bpf_debug_printk("grpc:server:uprobe/server_handleStream2: failed to read context type");
        return -3;
    }

    rc = bpf_probe_read_user(&go_context.data, sizeof(go_context.data), get_go_interface_instance(stream_ptr + stream_ctx_pos));
    if (rc != 0) {
        bpf_debug_printk("grpc:server:uprobe/server_handleStream2: failed to read context data");
        return -4;
    }
...
int uprobe_server_handleStream2_Returns(struct pt_regs *ctx) {
    bpf_debug_printk("=== uprobe_server_handleStream2_Returns called with ctx: %p ===", ctx);
    u64 server_stream_pos = 4;
    void *key = NULL;
    void *server_stream_ptr = get_argument(ctx, server_stream_pos);
    if (server_stream_ptr == NULL) {
    ...
    long rc = bpf_probe_read_user(&stream_ptr, sizeof(stream_ptr), (void *)(server_stream_ptr + server_stream_stream_pos));
    if (rc != 0) {
        bpf_debug_printk("grpc:server:uprobe/server_handleStream2Return: failed to read stream_ptr");
        return -2;
    }

lookup:
    key = (void *)GOROUTINE(ctx); //This is the consistent key.
    struct grpc_request_t *event = bpf_map_lookup_elem(&grpc_events, &key);
    if (event == NULL) {
        bpf_debug_printk("grpc:server:uprobe/server_handleStream2Return: event is NULL");
        return -5;
    }
...
{
    bpf_debug_printk("=== uprobe_http2Server_operateHeader called with ctx: %p ===", ctx);
    ...
    // Define the keys as arrays with fixed sizes
    char ckroute_key[CKR_KEY_LENGTH] = "ck-route";
    ...
    for (s32 i = 0; i < MAX_HEADERS; i++)
    {
        ...
        if (hf.name.len == CKR_KEY_LENGTH && hf.value.len == TRACE_ID_STRING_SIZE)
        {
            char current_key[CKR_KEY_LENGTH];
            bpf_probe_read(current_key, sizeof(current_key), hf.name.str);
            if (bpf_memcmp(ckroute_key, current_key, sizeof(ckroute_key)))
            {
                bpf_debug_printk("uprobe_http2Server_operateHeader: found ck-route header in incoming headers");
    ...
    if (found_ckroute) {
        bpf_debug_printk("uprobe_http2Server_operateHeader: ckroute value : %s", grpcReq.sc.TraceID); 
        bpf_debug_printk("uprobe_http2Server_operateHeader: update map : streamid_to_grpc_events , key: strteam_id %p, value type: grpc_request_t", stream_id );   
        if(bpf_map_update_elem(&streamid_to_grpc_events, &stream_id, &grpcReq, 0) != 0) {
            bpf_debug_printk("uprobe_http2Server_operateHeader: failed to update map : streamid_to_grpc_events , key: strteam_id %p, value type: grpc_request_t", stream_id );   
        }
    }
...
int uprobe_http2Server_WriteStatus(struct pt_regs *ctx) {
    bpf_debug_printk("=== uprobe_http2Server_WriteStatus called with ctx: %p ===", ctx);
...
int uprobe_http2Server_WriteStatus2(struct pt_regs *ctx) {
    bpf_debug_printk("=== uprobe_http2Server_WriteStatus2 called with ctx: %p ===", ctx);
    u64 server_stream_pos = 2;
    void *server_stream_ptr = get_argument(ctx, server_stream_pos);
    if (server_stream_ptr == NULL) {
        bpf_debug_printk("grpc:server:uprobe/http2Server_WriteStatus2: failed to get ServerStream arg");
        return -1;
    }
    ...
    long rc = bpf_probe_read_user(&stream_ptr, sizeof(stream_ptr), (void *)(server_stream_ptr + server_stream_stream_pos));
    if (rc != 0) {
        bpf_debug_printk("grpc:server:uprobe/http2Server_WriteStatus2: failed to read stream_ptr");
        return -2;
    }
