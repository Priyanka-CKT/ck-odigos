...
func New(logger *slog.Logger, ver string, serviceName string) probe.Probe {
	id := probe.ID{
...
func processFnC(e *event, logger *slog.Logger, serviceName string) telemetry.PathKey {
	logger.Info("=== Processing GRPC server event", "event", e)
	if e == nil {
		logger.Info("=== GRPC server event is nil")
		return telemetry.PathKey{}
	}
	parentTraceId := e.ParentSpanContext.TraceID.String()
	ckRoute := e.SpanContext.TraceID.String()
	if ckRoute == "" {
		logger.Info("=== GRPC server event has no trace ID")
		return telemetry.PathKey{}
	}
	...
	if method == "" {
		logger.Info("=== GRPC server event has empty method")
		return telemetry.PathKey{}
	}
	...
	logger.Info("=== Created GRPC Server PathKey", "pathKey", pathKey)
	return pathKey
}
