	...
	return &probe.SpanProducer[bpfObjects, event]{
		Base: probe.Base[bpfObjects, event]{
			...
			Consts: []probe.Const{
				...
				probe.StructFieldConst{
					Key: "chunk_writer_pos",
					Val: structfield.NewID("std", "net/http", "response", "cw"),
				},
				probe.StructFieldConst{
					Key: "chunk_writer_header_pos",
					Val: structfield.NewID("std", "net/http", "chunkWriter", "header"),
				},
				probe.StructFieldConst{
					Key: "buckets_ptr_pos",
			...
			Uprobes: []probe.Uprobe{
				...
				},
				{
					...
					EntryProbe: "uprobe_writeSubset_server",
					...
					DependsOn: []string{"net/http.HandlerFunc.ServeHTTP", "net/http.Header.writeSubset"},
				},
			},
			SpecFn: loadBpf,
		},
...
func processFnC(e *event, logger *slog.Logger, serviceName string) telemetry.PathKey {
	// Protect against nil event
	if e == nil {
		if logger != nil {
			logger.Error("=== Received nil event in HTTP server processFnC")
		}
	...
	logger.Info("=== Processing HTTP server event", "event", e)
	...
	if parentTraceId == "" {
		parentTraceId = "00000000000000000000000000000000"
	}

	path := unix.ByteSliceToString(e.Path[:])
	patternPath := unix.ByteSliceToString(e.PathPattern[:])
	if isPatternPathSupported {
		patternPath, err := http.ParsePattern(patternPath)
		if err != nil || patternPath == "" {
			logger.Info("=== Invalid pattern path", "patternPath", patternPath)
		} else {
			path = patternPath
		}
	} else {
		if patternPath != "" {
			logger.Info("=== Using pattern path", "patternPath", patternPath)
			path = patternPath
		}
	}

	pathKey := telemetry.PathKey{
		PathKey: e.SpanContext.TraceID.String(),
		GraphPathNode: telemetry.GraphPathNode{
			IncomingPath: &parentTraceId,
			Runtime:      "go",
			GraphPathElement: telemetry.HttpServiceElement{
				Type:       "HttpService",
				HttpMethod: unix.ByteSliceToString(e.Method[:]),
				HttpRoute:  path,
			},
			StartTime: e.StartTime,
			EndTime:   e.EndTime,
			IsError:   e.StatusCode >= 500 && e.StatusCode < 600, //We are only considering 5xx errors for now
			ErrorCode: strconv.FormatUint(e.StatusCode, 10),
		},
	}

	logger.Info("=== Created HTTP Server PathKey", "pathKey", pathKey)
	return pathKey
}
