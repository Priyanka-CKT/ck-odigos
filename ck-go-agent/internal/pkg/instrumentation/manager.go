...
import (
	...
	mux "go.opentelemetry.io/auto/internal/pkg/instrumentation/bpf/github.com/gorilla/mux"
	redisV9 "go.opentelemetry.io/auto/internal/pkg/instrumentation/bpf/github.com/redis/go-redis/v9"
...
type Manager struct {
	...
	serviceName         string
	clientOverridePaths string
}

// NewManager returns a new [Manager].
func NewManager(logger *slog.Logger, otelController *opentelemetry.Controller, globalImpl bool, cp ConfigProvider, version string, factory *telemetry.Factory, serviceName string, clientOverridePaths string) (*Manager, error) {
	m := &Manager{
		logger:              logger,
		version:             version,
		...
		clientOverridePaths: clientOverridePaths,
	}
...
func (m *Manager) applyConfig(c Config) error {
	...
	for id, p := range m.probes {
		...
		if !currentlyEnabled && newEnabled {
			...
			err = errors.Join(err, p.Load(m.exe, m.td, c.SamplingConfig, m.serviceName))
			if err == nil {
				m.runProbe(p)
			}
			continue
...
func (m *Manager) runProbe(p probe.Probe) {
	...
	go func(ap probe.Probe) {
		...
		ap.Run(m.otelController.Trace, m.factory.Export)
	}(p)
}
...
func (m *Manager) Run(ctx context.Context) error {
	m.logger.Debug("==== Going to run probes from manager..")
...
func (m *Manager) loadProbes(target *process.TargetDetails) error {
	...
	for name, i := range m.probes {
		if isProbeEnabled(name, m.currentConfig) {
			...
			err := i.Load(exe, target, m.currentConfig.SamplingConfig, m.serviceName)
			if err != nil {
...
func (m *Manager) cleanup(target *process.TargetDetails) error {
	...
	// Shutdown factory
	if m.factory != nil {
		err = errors.Join(err, m.factory.Shutdown())
	}
...
func (m *Manager) availableProbes() []probe.Probe {
	p := []probe.Probe{
		grpcClient.New(m.logger, m.version, m.serviceName),
		grpcServer.New(m.logger, m.version, m.serviceName),
		httpServer.New(m.logger, m.version, m.serviceName),
		httpClient.New(m.logger, m.version, m.serviceName),
		dbSql.New(m.logger, m.version, m.serviceName),
		redisV9.New(m.logger, m.version, m.serviceName),
		redisV9.NewV8(m.logger, m.version, m.serviceName),
		kafkaProducer.New(m.logger, m.version, m.serviceName),
		kafkaConsumer.New(m.logger, m.version, m.serviceName),
		mux.New(m.logger, m.version, m.serviceName),
