...
func manifests() ([]inspect.Manifest, error) {
	...
	goRedisVers, err := PkgVersions("github.com/redis/go-redis/v9")
	if err != nil {
		return nil, fmt.Errorf("failed to get \"github.com/redis/go-redis/v9\" versions: %w", err)
	}

	goRedisV8Vers, err := PkgVersions("github.com/go-redis/redis/v8	")
	if err != nil {
		return nil, fmt.Errorf("failed to get \"github.com/go-redis/redis/v8\" versions: %w", err)
	}

	gorillaMuxVers, err := PkgVersions("github.com/gorilla/mux")
	if err != nil {
		return nil, fmt.Errorf("failed to get \"github.com/gorilla/mux\" versions: %w", err)
	}
	...
	return []inspect.Manifest{
		...
		},
		{
			...
			StructFields: []structfield.ID{
				...
				structfield.NewID("std", "net/http", "Response", "Header"),
				structfield.NewID("std", "net/http", "response", "req"),
				structfield.NewID("std", "net/http", "response", "status"),
				structfield.NewID("std", "net/http", "response", "cw"),
				structfield.NewID("std", "net/http", "chunkWriter", "header"),
		...
		},
		{
			...
			StructFields: []structfield.ID{
				...
				structfield.NewID("google.golang.org/grpc", "google.golang.org/grpc/internal/transport", "loopyWriter", "conn"),
		...
		{
			Application: inspect.Application{
				Renderer: ren("templates/github.com/redis/go-redis/v9/*.tmpl"),
				Versions: goRedisVers,
			},
			StructFields: []structfield.ID{
				structfield.NewID("github.com/redis/go-redis/v9", "github.com/redis/go-redis/v9/internal/pool", "Conn", "bw"),
				structfield.NewID("github.com/redis/go-redis/v9", "github.com/redis/go-redis/v9", "baseCmd", "args"),
			},
		},
		{
			Application: inspect.Application{
				Renderer: ren("templates/github.com/go-redis/redis/v8/*.tmpl"),
				Versions: goRedisV8Vers,
			},
			StructFields: []structfield.ID{
				structfield.NewID("github.com/go-redis/redis/v8", "github.com/go-redis/redis/v8/internal/pool", "Conn", "bw"),
				structfield.NewID("github.com/go-redis/redis/v8", "github.com/go-redis/redis/v8", "baseCmd", "args"),
			},
		},
		{
			Application: inspect.Application{
				Renderer: ren("templates/github.com/gorilla/mux/*.tmpl"),
				Versions: gorillaMuxVers,
			},
			StructFields: []structfield.ID{
				structfield.NewID("github.com/gorilla/mux", "github.com/gorilla/mux", "RouteMatch", "Route"),
				structfield.NewID("github.com/gorilla/mux", "github.com/gorilla/mux", "Route", "regexp"),
				structfield.NewID("github.com/gorilla/mux", "github.com/gorilla/mux", "Router", "regexp"),
				structfield.NewID("github.com/gorilla/mux", "github.com/gorilla/mux", "routeConf", "regexp"),
				structfield.NewID("github.com/gorilla/mux", "github.com/gorilla/mux", "Route", "routeConf"),
				structfield.NewID("github.com/gorilla/mux", "github.com/gorilla/mux", "Router", "routeConf"),

				structfield.NewID("github.com/gorilla/mux", "github.com/gorilla/mux", "routeRegexpGroup", "path"),
				structfield.NewID("github.com/gorilla/mux", "github.com/gorilla/mux", "routeRegexp", "template"),
				structfield.NewID("github.com/gorilla/mux", "github.com/gorilla/mux", "routeRegexp", "reverse"),
			},
		},
	}, nil
}
...
func run() error {
	...
	to, err := i.Do(ctx)
	if err != nil {
		logger.Error("failed to get offsets", "error", err)
		// Even if there's an error, try to write partial results if we have any
		if to != nil {
			logger.Info("writing partial offsets due to error", "dest", outputFile)
			if err := writeOffsets(to, outputFile); err != nil {
				logger.Error("failed to write partial offsets", "error", err)
			}
		}
		return err
	}
	...
	return writeOffsets(to, outputFile)
}

// writeOffsets writes the offsets to the specified output file
func writeOffsets(to interface{}, outputFile string) error {
