...
export type NodePositions = Partial<Record<
...
export const getNodePositions = ({ containerWidth }: Params) => {
  const startX = 24;
  const endX = (containerWidth <= 1500 ? 1500 : containerWidth) - nodeConfig.nodeWidth - startX;
  const getY = (idx?: number) => nodeConfig.nodeHeight * ((idx || 0) + 1);
